...
export default function BulkOutgoingInvoicesPage() {
  ...
  async function handleFilesSelect(files: File[]) {
    ...
    try {
      const processedItems = await processBulkQRCodes(files, {
        ...
        isOutgoing: true, // GİDEN FATURA için flag
      ...
      console.error('Error processing bulk outgoing invoices:', error);
  ...
  async function handleCustomerNameChange(vkn: string, customerName: string) {
    if (!company) return;
    
    // Update all items with the same VKN
    setItems(prevItems =>
      prevItems.map(item => {
        if (item.vkn === vkn) {
          const updatedItem = { ...item, supplier_name: customerName };
          validateBulkInvoiceItem(updatedItem);
          return updatedItem;
        }
        return item;
    ...
    try {
      ...
      for (const item of items) {
        try {
          ...
          // 2. Create or get customer (GİDEN FATURA için müşteri)
          let customerId: string | null = null;
          if (item.vkn && item.supplier_name) {
            try {
              const customer = await getOrCreateCustomer(item.vkn, item.supplier_name, company.id);
              if (customer) {
          ...
          const { error: insertError } = await supabase
            ...
            .insert({
              ...
              customer_name: item.supplier_name, // supplier_name aslında customer_name
          ...
          console.error('Error inserting outgoing invoice:', {
      ...
      if (failCount === 0) {
        alert(`✅ ${successCount} giden fatura başarıyla eklendi!`);
        router.push('/invoices/outgoing');
      } else {
        alert(
      ...
      console.error('Error submitting bulk outgoing invoices:', error);
    ...
    <Sidebar>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white rounded-lg shadow-sm p-4 md:p-6">
          <div>
            <h2 className="text-lg md:text-xl font-semibold text-gray-900">Toplu Giden Fatura Ekle</h2>
            <p className="mt-1 text-xs md:text-sm text-gray-500">
              Birden fazla giden faturayı aynı anda yükleyin ve QR kodlarından bilgileri otomatik okutun
            </p>
          </div>
        ...
        {isProcessing && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <p className="mt-3 text-sm text-blue-700">
              QR kodları okunuyor ve müşteri bilgileri dolduruluyor...
            </p>
          </div>
        )}
        ...
        {items.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            <div className="p-3 md:p-4 border-b border-gray-200">
              <h3 className="text-base md:text-lg font-medium text-gray-900">Giden Fatura Listesi</h3>
            </div>
            ...
            <div className="overflow-x-auto max-h-[60vh] overflow-y-auto">
              <BulkInvoiceTable
                ...
                onSupplierNameChange={handleCustomerNameChange}
                isOutgoing={true}
              />
            </div>
            ...
            <div className="flex items-start gap-2 md:gap-3">
              ...
              <div className="flex-1">
                ...
                <ul className="text-xs md:text-sm text-blue-800 space-y-1 list-disc list-inside">
                  <li>Aynı VKN&apos;ye sahip faturalar için bir tanesine müşteri adı girdiğinizde, diğer faturalar otomatik olarak güncellenecektir.</li>
